...
| `flags` | 1 byte | bitfield | bit 0: `COMPACT_SIG` — MUST be `1` if and only if `sigAlg = 0x02` (Ed25519), and `0` if and only if `sigAlg = 0x01` (RSA-SHA256); a mismatch between `flags` bit 0 and `sigAlg` MUST result in rejection with `V4005`; bits 1–7: reserved, MUST be zero |
...
| `sigAlg` | 1 byte | u8 | `0x01` = RSA-SHA256, `0x02` = Ed25519 |
...
|---|---|---|:---:|---|
| `0x01` | `alg` | enum(u8) | REQUIRED | `0x01` = RSA-SHA256, `0x02` = ECDSA-SHA256 |
